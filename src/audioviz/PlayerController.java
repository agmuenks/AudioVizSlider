...
public class PlayerController implements Initializable {
    ...
    private void handleReady() {
        ...
        lengthText.setText(String.format("%.1f ms", durationMs));
        Duration ct = mediaPlayer.getCurrentTime();
        double ctMs = ct.toMillis();
        currentText.setText(String.format("%.1f ms", ctMs));
        currentVisualizer.start(numBands, vizPane);
        timeSlider.setMin(0);
    ...
    @FXML 
    private void handleSliderClicked(Event event){
        if(mediaPlayer != null){
            mediaPlayer.pause();
        }
    }
    ...
    @FXML
    private void handleSliderReleased(Event event){
        if (mediaPlayer != null) {
            double updatedMs = timeSlider.getValue();
            Duration updatedDuration = new Duration(updatedMs);
            mediaPlayer.seek(updatedDuration);
            mediaPlayer.play();
        }
    }
}
